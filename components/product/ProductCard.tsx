...
export default function ProductCard({ product, variant = 'light' }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className={`group block rounded-lg md:rounded-xl lg:rounded-2xl shadow-sm hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1 ${
        isDark
          ? 'product-card-dark'
    ...
    >
      ...
      <div className={`aspect-square relative overflow-hidden ${
        isDark ? 'bg-[hsl(var(--profile-bg-secondary))]' : 'bg-gradient-to-br from-gray-50 to-gray-100'
      }`}>
        <img
          src={imageUrl}
          alt={product.name}
      ...
      <div className="p-3 md:p-4">
        {/* Title */}
        <h3 className={`text-xs md:text-sm font-medium line-clamp-2 mb-1.5 md:mb-2 transition-colors min-h-[2rem] md:min-h-[2.5rem] ${
          isDark
            ? 'text-[hsl(var(--profile-text-primary))] group-hover:text-gold'
        ...
        {product.rating_count > 0 && (
          <div className="flex items-center gap-1 md:gap-1.5 mb-2 md:mb-3">
            ...
            <span className={`text-[10px] md:text-xs font-medium ${
              isDark ? 'text-[hsl(var(--profile-text-muted))]' : 'text-gray-500'
            }`}>
        ...
        <div className="mb-2 md:mb-3">
          <div className="flex items-baseline gap-1 md:gap-1.5">
            ...
            <span className={`text-xl md:text-2xl font-bold ${
              isDark ? 'price-gold' : 'bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent'
            }`}>
              {product.price.toFixed(2).split('.')[0]}
            </span>
            <span className={`text-xs md:text-sm ${
              isDark ? 'text-[hsl(var(--profile-text-secondary))]' : 'text-gray-500'
            }`}>
          ...
          {product.original_price && product.original_price > product.price && (
            <div className="flex items-center gap-1.5 md:gap-2 mt-0.5 md:mt-1">
              <span className={`text-[10px] md:text-xs line-through ${
                isDark ? 'text-[hsl(var(--profile-text-muted))]' : 'text-gray-400'
              }`}>
                ¥{product.original_price.toFixed(2)}
              </span>
              <span className={`text-[10px] md:text-xs font-semibold ${
                isDark ? 'text-[hsl(var(--profile-amber))]' : 'text-red-500'
              }`}>
